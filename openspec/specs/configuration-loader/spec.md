...
The system SHALL resolve which action, if any, matches a dispatched event context.

#### Scenario: Matching rule exists
- **WHEN** a DispatchContext is provided
- **AND** a Profile exists for the application in the context
- **AND** the Profile contains a mapping for the event
- **THEN** the system returns the corresponding Action

#### Scenario: No profile for application
- **WHEN** a DispatchContext is provided
- **AND** no Profile exists for the application in the context
- **THEN** the system reports that no action is configured

#### Scenario: No mapping for event
- **WHEN** a DispatchContext is provided
- **AND** a Profile exists for the application
- **AND** the Profile does not contain a mapping for the event
- **THEN** the system reports that no action is configured
